...
import { TrendingUp, Printer, Search, FileSpreadsheet } from "lucide-react";
import { Btn } from "@/components/shared/Btn";
import { Combobox } from "@/components/shared/Combobox";
import { DateInput } from "@/components/shared/DateInput";
import { ModuleHeader } from "@/components/shared/ModuleHeader";
import { fmtM } from "@/lib/format";
import { getBordereauProduction, type BordereauProductionPayload } from "@/services/bordereaux.service";
import { openBordereauProduction } from "@/services/documents.service";
...
// Bordereau de Production / Émission de primes et de commissions (2026-08)
// — voir demande utilisateur. Groupé par compagnie — TOUTES les compagnies
// apparaissent, même sans mouvement sur la période (voir modèle fourni).
// Une compagnie précise peut aussi être choisie pour générer le bordereau
// destiné à cette seule compagnie (voir demande utilisateur : "chaque type
// de bordereau doit pouvoir être généré par compagnie").
export default function BordereauProductionView() {
  ...
  const handleRechercher = async () => {
    setRecherchant(true);
    try {
      setPayload(await getBordereauProduction(du || undefined, au || undefined, compagnieId || undefined));
    } finally {
  ...
  return (
    <div className="p-6">
      <ModuleHeader title="Bordereau de Production" subtitle="Émission de primes et de commissions, par compagnie — États CIMA" icon={TrendingUp} />
      ...
      <div className="bg-card border border-border rounded-xl p-4 mb-4">
        <div className="flex items-end gap-3 flex-wrap">
          ...
          <Btn variant="ghost" onClick={() => openBordereauProduction(du || undefined, au || undefined, compagnieId || undefined)}><Printer className="w-4 h-4" />Imprimer ce bordereau</Btn>
          <Btn variant="ghost" onClick={() => openBordereauProduction(du || undefined, au || undefined, compagnieId || undefined, "xlsx")}><FileSpreadsheet className="w-4 h-4" />Excel</Btn>
        </div>
      </div>
                ...
                <table className="w-full text-[12px]">
                  <thead>
                    <tr className="border-b border-border bg-secondary/20">
                      {["N° Police", "Code Assuré", "Num Quittance", "Date émis. Quit.", "Date avenant", "Nom Souscripteur/Assuré", "Date début", "Date fin", "Produit", "Capitaux", "Primes", "Access.", "Taxes", "Primes Tot.", "Commis."].map((h) => (
