...
export default function FinanceDashboard() {
  ...
  const aReverser = commissions.filter((c) => c.statut === "À reverser").reduce((a, b) => a + b.montantCommission, 0);

  return (
    <div className="p-6 space-y-6">
      <ModuleHeader title="Tableau de Bord — Finance" subtitle="Trésorerie, commissions et recouvrement" icon={Wallet} />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Solde consolidé" value={`${fmtM(soldeConsolide)} XAF`} icon={Wallet} />
        <StatCard title="Commissions à reverser" value={`${fmtM(aReverser)} XAF`} icon={DollarSign} accent="bg-amber-500/10" />
        <StatCard title="Total impayés" value={`${fmtM(totalImpayes)} XAF`} icon={CreditCard} accent="bg-red-500/10" />
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-border/50">
          {enCours.slice(0, 6).map((i) => (
            <div key={i.id} className="flex items-center justify-between px-4 py-3 hover:bg-secondary/30 transition-colors">
              <div>
              ...
              <div className="text-right space-y-1">
                <p className="text-xs font-semibold text-foreground" style={{ fontFamily: "'DM Mono', monospace" }}>{fmtM(i.montantDu)} XAF</p>
                <div className="flex items-center gap-2 justify-end">
                  <span className="text-xs text-red-400" style={{ fontFamily: "'DM Mono', monospace" }}>{i.joursRetard} j</span>
                  <Badge variant={niveauVariant[i.niveau] ?? "neutral"}>{i.niveau}</Badge>
                </div>
              </div>
            </div>
          ))}
          {enCours.length === 0 && <p className="px-4 py-6 text-sm text-muted-foreground text-center">Aucun impayé en cours</p>}
        </div>
      </div>
    </div>
  );
